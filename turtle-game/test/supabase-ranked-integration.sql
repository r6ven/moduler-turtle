...
begin
  ...
  v_primary_token :=
    public._issue_player_session('ranked_ci_primary')
      ->> 'session_token';

  v_other_token :=
    public._issue_player_session('ranked_ci_other')
      ->> 'session_token';

  -- Sezon önce taslak olarak oluşturulur.
  ...
  select
    ...
    series.slot,
    'ci-profile-' || series.slot,
    series.slot,
    series.slot,
    'ranked-v2-ci-' || series.slot,
    2,
    1,
    'ranked-v2',
    'gameplay-ci-' || series.slot,
    'presentation-ci-' || series.slot,
    jsonb_build_object(
      'schemaVersion', 1,
  ...
  -- Slotlar hazırlandıktan sonra sezon yayınlanır.
  ...
  -- Uyumsuz istemci günlük hakkı tüketmemeli.
  ...
  if v_response ->> 'code' <> 'client_update_required' then
    raise exception
      'incompatible client was not rejected: %',
      v_response;
  end if;
  ...
  where username = 'ranked_ci_primary';

  if v_attempt_count <> 0 then
    raise exception
      'compatibility check consumed the daily attempt';
  end if;

  -- Uyumlu istemci ilk puzzle ile başlamalı.
  v_response := public.start_ranked_attempt(
    v_primary_token,
    array[1],
    array['ranked-v2']
  ...
  if coalesce(
     ...
     or not (v_response ? 'puzzle')
     or v_response ? 'puzzles'
     or (
       v_response -> 'puzzle' ->> 'slot'
     )::integer <> 1
  then
    raise exception
      'atomic start leaked or omitted puzzle slots: %',
      v_response;
  end if;

  v_attempt_id :=
    (v_response ->> 'attempt_id')::uuid;
  ...
  if v_repeat ->> 'attempt_id' <> v_attempt_id::text
     or v_repeat -> 'puzzle' ->> 'released_at'
        <> v_released_at
  then
    raise exception
      'start retry reset attempt identity or release time';
  end if;

  -- İlk puzzle doğrulanmadan ikinci puzzle açılamamalı.
  ...
  if v_repeat ->> 'code' <> 'invalid_slot' then
    raise exception
      'future slot was released before verification: %',
      v_repeat;
  end if;

  -- Başka oyuncu replay bağlamını görememeli.
  ...
  if v_context ->> 'code' <> 'attempt_not_found' then
    raise exception
      'another player could inspect replay context: %',
      v_context;
  end if;

  -- Attempt sahibi replay bağlamını alabilmeli.
  v_context := public.get_ranked_replay_context(
    v_primary_token,
    v_attempt_id,
    1
  ...
  -- Birinci puzzle sonucu kabul edilmeli.
  ...
  if coalesce(
       (v_accepted ->> 'ok')::boolean,
       false
     ) is not true
  then
    raise exception
      'verified replay was not accepted: %',
      v_accepted;
  end if;
  ...
  if coalesce(
       (v_repeat ->> 'duplicate')::boolean,
       false
     ) is not true
  then
    raise exception
      'submission retry was not idempotent: %',
      v_repeat;
  end if;
  ...
  then
    raise exception
      'next verified slot was not released: %',
      v_response;
  end if;
  ...
  if v_repeat -> 'puzzle' ->> 'released_at'
     <> v_released_at
  then
    raise exception
  ...
  if coalesce(
     ...
     or coalesce(
       (v_response ->> 'score_eligible')::boolean,
       true
     ) is not false
  then
    raise exception
      'current slot was not forfeited: %',
      v_response;
  end if;
  ...
  if v_repeat ->> 'forfeit_reason' <> 'menu_opened' then
    raise exception
      'slot forfeit retry was not idempotent: %',
      v_repeat;
  end if;

  -- Puan dışı puzzle yine çözülmeli ve doğrulanmalı.
  v_accepted := public.accept_ranked_replay(
    v_primary_token,
    v_attempt_id,
    2,
  ...
  if coalesce(
     ...
     or coalesce(
       (v_accepted ->> 'score_eligible')::boolean,
       true
     ) is not false
  then
    raise exception
      'forfeited slot was not solved as score-ineligible: %',
      v_accepted;
  end if;
  ...
  then
    raise exception
      'next slot did not return to ranked scoring: %',
      v_response;
  end if;

  -- Özel replay fonksiyonları public rollere açık olmamalı.
  ...
  then
    raise exception
      'private replay RPCs are exposed to public roles';
  end if;

  -- Yayınlanmış puzzle artık değiştirilememeli.
  begin
    ...
    raise exception
      'published puzzle mutation unexpectedly succeeded';
  exception
